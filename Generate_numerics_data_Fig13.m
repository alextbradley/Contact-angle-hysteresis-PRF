...
%% Smaller values of \lambda_max
%Parameters
hysparmax_out = [0.005:0.005:0.05];
nu_out = 1:1:6; %above 6 we run risk of trapping (nu \gtrsim 8)
V = 0.2;
xu0_escape = zeros(length(hysparmax_out), length(nu_out));
tend = 20; 
N = 30; %number of grid points in integration
theta_r = 0;
...
for i = 1:length(hysparmax_out)
    ...
    for j = 1:length(nu_out)
        ...
        if istouch == 2 %droplet was trapped, so perform the bisection
            ...
            while err > 1e-3 %accuracy of solutions specified by RHS of inequality
                
                
                %perform the integration
                logout = 0; %specify linear out (although doesn't make a difference)
                [~, ~, ~, ~, istouch] = run_droplet(nu,hysparmax, theta_r, V, xu0_guess,N, tend, logout);
        ...
        elseif istouch == 5 %escaped even at this value, so say always escapes
            ...
            xu0_escape(i,j) = -1; %set to -1 to identify
...
save('Data/numerics_comparison_data_smaller_hysparmax.mat', 'hysparmax_out', 'nu_out', 'V','xu0_escape')
%% Larger values of \lambda_{max}
%Parameters
hysparmax_out = [0.02:0.02:0.28];
...
save('Data/numerics_comparison_data_larger_hysparmax.mat', 'hysparmax_out', 'nu_out', 'V','xu0_escape')
